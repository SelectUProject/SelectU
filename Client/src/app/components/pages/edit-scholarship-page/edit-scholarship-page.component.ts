...
export class EditScholarshipPageComponent implements OnInit {
  ...
  async populateForm(): Promise<void> {
    await this._scholarshipService
      ...
      .then((response) => {
        ...
        this.orignalImage = response.imageURL;
  ...
  updateScholarship() {
    let editScholarshipForm = <ScholarshipUpdateDTO>(
      this.editScholarshipForm.value
    );

    editScholarshipForm.imageURL = this.orignalImage;
